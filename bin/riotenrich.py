...
@Configuration()
class RIOTEnrichCommand(EventingCommand):
    """
    Transforming command that enriches Splunk search events with the RIOT status of the IP addresses
    present as values in the IP field passed in ip_field parameter.
    Data pulled from: /v2/riot/{ip}

    **Syntax**::
    `index=firewall | riotenrich ip_field="ip"

    **Description**::
    The `riotenrich` command uses the IP represented by IP field in `ip_field` to return 
    context information using method :method:`riot` from GreyNoise Python SDK.
    """
    ip_field = Option(
        doc='''
    ...
    def transform(self, records):
        ...
        if self.search_results_info and not self.metadata.preview:
            ...
            try:
                ...
                if len(chunk_dict) > 0:    
                    for event in event_generator.get_all_events(api_client, 'riot', ip_field, chunk_dict, logger, threads=THREADS):
                        yield event
                ...
                logger.info("Exception occured while getting RIOT status for events, Error: {}".format(traceback.format_exc()))
                self.write_error("Exception occured while enriching events with the RIOT status of IP addresses. See greynoise_main.log for more details.")

    def __init__(self):
        super(RIOTEnrichCommand, self).__init__()

dispatch(RIOTEnrichCommand, sys.argv, sys.stdin, sys.stdout, __name__)
