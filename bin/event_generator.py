...
def pull_data_from_api(fetch_method, logger, params, api_sleep_timer=3):
    ...
    :param api_sleep_timer: Wait time in seconds before actual call to API to avoid ConnectionError with GreyNoise server
    ...
    try:
        ...
        logger.debug("Either the event doesn't have ip_field or value of IP address present in event is invalid, parameter passed to API: '{}'".format(params))
        msg = 'IP address doesn\'t match the valid IP format'
        return {
            'message': 'error',
            'response': msg
        }
        ...
        logger.error("Error while connecting to the Server. Please check your connection and try again. Exception: {}".format(str(e)))
        return {
            'message': 'error',
            'response': 'Connection error occured while retrieving information from GreyNoise API'
        }
    except RequestException as e:
        logger.error("There was an ambiguous exception that occurred while handling your Request. Please try again. Exception: {}".format(str(e)))
        return {
            'message': 'error',
...
def get_all_events(api_client, method, ip_field, chunk_dict, logger, threads=3):
    ...
    logger.info("Fetching noise status for {} chunk(s) with {} thread(s)".format(len(chunk_dict), threads))
    ...
    with ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        # Implementing the filter mechanism in the common code may increase time and cognitive complexity that is not required for other transforming commands
...
def method_response_mapper(method, result, logger):
    ...
    if method == 'enrich' or method == 'riot':
        ...
        raise Exception("Unexpected method type encountered while processing the API response, method name: {}".format(str(method)))

    return (generate_missing_events, result)
    
def event_processor(records_dict, result, method, ip_field, logger):
    """
    Process on each chunk, format the response retrieved from the API and Send the results of the transforming command to Splunk
    ...
    # This will either have API response for the chunk or the exception message denoting exception occured while fetching the data
        ...
        if error_flag:
            ...
            if ip_field in record and record[ip_field]:
                ...
                if isinstance(record[ip_field], six.string_types) and record[ip_field] in ip_lookup:
                    ...
                    # Meaning ip is either invalid or not returned by the API, happens when quick method is used while retrieving data
...
def make_valid_event(method, data, first_event=False, record={}):
    """
    ...
    if method in GENERATING_COMMAND_METHODS:
        ...
        if first_event:
            ...
            # Get the fields from the response json and put them into json so that Splunk can get the values of the fields from it
...
def make_invalid_event(method, data, first_event=False, record={}):
    """
...
def batch(iterable, ip_field, events_per_chunk, logger, optimize_requests=True):
    ...
    for record in iterable_list:
        ...
        if ip_field in record and record[ip_field]:
            if isinstance(record[ip_field], six.string_types):
